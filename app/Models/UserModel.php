...
class UserModel
{
    ...
    public function login($username, $password)
    {
        ...
        $lastLogin = $db->table('st_table_users')->update(['last_login' => time()], ['record_id' => $user->record_id]);
        return $user ?? false;
    }
    ...
    public function editRecord($record_id, $data = [])
    {
        $db = db_connect();
        if (empty($data)) {
            return $result = $db->table('st_table_record')->where('record_id', $record_id)->get()->getRow();
        }
        $array = [
            'name' => $data['name'],
            'position' => $data['position'],
        ...
        $result = $db->table('st_table_record')->update($array, ['record_id' => $record_id]);
        if ($result) {
    ...
    public function deleteRecord($record_id)
    {
        ...
        $result = $db->table('st_table_record')->delete(['record_id' => $record_id]);
        if ($result) {
